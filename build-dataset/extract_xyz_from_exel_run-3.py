...
EXCEL_PATH = PROJECT_ROOT / "dataset" / "100_results_run-3.xlsx"
OUT_ROOT = PROJECT_ROOT / "data" / "raw" / "xyz_graphs"
SKIPPED_LOG_PATH = PROJECT_ROOT / "data" / "metadata" / "logs" / "extract_run_3_skipped_rows.txt"
COUNTS_PATH = PROJECT_ROOT / "data" / "metadata" / "liczbagrafow.txt"
...
def next_index_for_type(out_root: Path, gtype: str) -> int:
    ...
    na podstawie istniejących już plików.
    """
...
def count_existing_graphs(out_root: Path) -> dict[str, int]:
    """
    Liczy wszystkie aktualnie istniejące pliki .xyz per klasa
    w katalogu OUT_ROOT.
    """
    counts: dict[str, int] = {}
    ...
    for class_dir in out_root.iterdir():
        if not class_dir.is_dir():
            continue

        count = sum(1 for _ in class_dir.glob("*.xyz"))
        counts[class_dir.name] = count

    return counts


def write_counts_file(counts_path: Path, counts: dict[str, int]) -> None:
    """
    Zapisuje liczbę grafów per klasa do osobnego pliku tekstowego.
    """
    ...
    with counts_path.open("w", encoding="utf-8") as f:
        f.write("Liczba grafów per klasa\n")
        f.write("=======================\n\n")

        for gtype in sorted(counts):
            f.write(f"{gtype}: {counts[gtype]}\n")
...
def main() -> None:
    ...
    final_counts = count_existing_graphs(OUT_ROOT)
    write_counts_file(COUNTS_PATH, final_counts)

    print(f"Excel: {EXCEL_PATH}")
    print(f"Katalog wyjściowy: {OUT_ROOT}")
    print(f"Zapisano nowych plików .xyz: {written}")
    ...
    print(f"Liczba grafów per klasa zapisana do: {COUNTS_PATH}")

    if final_counts:
        print("\nAktualna liczba grafów per klasa:")
        for gtype in sorted(final_counts):
            print(f"  {gtype}: {final_counts[gtype]}")
